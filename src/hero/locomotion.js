...
// Targets (internal tuning notes): run 4.3-4.9 H/s with no ramp, 180° turn ≤ 6 sf, 5.3 steps/s,
...
export const LOCO = {
  ...
  hoverSink: 0.1,       // m/s: an air string holds the apex (SENJIN tuning: ~10 swings in ~2.4 s at about apex height)
...
/** Run cadence in steps/s for a ground speed (m/s): 5.3 at top speed (SENJIN tuning), ≈3.4 at a walk. Shared with the pose. */
...
export function stepPhysics(h) {
  ...
  if (h.move === 'dash' && h.moveT === DASH_PLANT && h.grounded) {       // SENJIN tuning: the lunge thrust lands in a dust cloud
  ...
  if (m && m.air && !m.landFrame) {
    // air string (tuning target: SENJIN tuning A→X×n hangs at about apex height for ~2.4 s, then a ≈0.4 s spread-arm fall): the jump
