...
//               follow-through (SENJIN tuning: N1 trail f163–166 → C2 flash f167, N2 f294–297 → C3 f299, N3 f53–56 → C4 f57)
...
// resolve in blade order (`sweep`), so a normal catches ~3–8 of a packed ring one or two at a time (tuning target: +1…+4 per
// swing, SENJIN tuning sparks on 1–3 soldiers) and a finisher still clears 15–45.
//
// Timing targets (internal tuning notes, combo-system): N1–N6 hit onsets 25, 26, 24, 26 sf apart, then the late N6 accent
// 35 sf after N5 (SENJIN tuning audio onsets 26, 28, 22, 26, 34), with a held finish pose. Onset spacing = cancel_k + tell_k+1 −
...
// Charges are a commitment, not a flick (internal tuning notes): tell → strike → HELD pose → delayed ground wave
// → recovery, so start → idle incl. hitstop lands in the ±20 % band of SENJIN tuning: C1 64, C2 126 (vault apex ≈ 3 m),
...
export const MOVES = {
  ...
  // N4 run-in, overhead carry round behind the head, shoulder-high sweep right → left (the string's first surge); N3 flows straight into a 14 sf run-in (tuning target: 16 sf)
  ...
  // N5 over-shoulder windup (12 sf, tuning target: 16), chop, twirl whipped out level, whip round into N6's late accent
  ...
  // mash restarts the string; idle 48 sf after N6 starts (tuning target: finish hold 9 f + return 7 f)
  ...
  // (tuning target: thrust at 28 sf, back in stance at 64)
  ...
  // (1.6 H) at ≈ 54, air sweeps around the apex, touchdown ≈ 82, held landing crouch (tuning target: vault 42→90, neutral 126)
  ...
  // pillar ring erupts ~20 sf later (tuning target: twirl 16–56, slam 58–74, dark arc, pillars 96–124, neutral 142)
  ...
  // return (tuning target: sweep 26–34, held f74–86 = 26 sf, neutral 86). Window end e = 52 keeps the clip's hold key (e − 4)
  ...
  // launches everything around (tuning target: low sweep 20–40, upswing 42–50, ray fan at 52, neutral 90).
  ...
  // outward (tuning target: twirl 32–92, leap 94, plunge 102, eruption 108–164, neutral 168)
  ...
  // thrust and the recovery (tuning target: 3 spins ≈ 16 sf apart, lunge thrust with dust, neutral ≈ 94). A mobility move: a
  ...
  // hang ≈ 26 sf at the apex, plunge in ≤ 3 sf, shockwave on landing, crouch (tuning target: SENJIN tuning A→Y apex 1.3–1.6 H at ≈ 27 sf,
...
export const AIR_CHAIN_MAX = 10;  // swipes per jump (the rapid SENJIN tuning jump attack shows ~10 over 2.9 s; locomotion-dodge r2: 8 → 10)
